# Supplemental Figure — DL discrimination by PTC subclass at n = 819
...
NMD predictor, restricted to the n = 819 ref-AUG-traceable subset
...
|---|---|---|---|
| NMD+/PTC+ vs Control | 0.95 | 0.93 | 195 vs 204 |
| NMD+/PTC− vs Control | 0.69 | 0.13 | 16 vs 204 |

Mean predicted NMD probability per subclass:
- Control: 0.18
- NMD+/PTC+: 0.84
- NMD+/PTC−: 0.28 (below the 0.5 decision threshold)
...
paralog-free test split. The NMD+/PTC− subgroup has n = 16 at the test
...
| `data/predprob_by_subclass_refaug.tsv` | Per-isoform predicted probability + group (shared with `SF40_PTCSubclassBranchSHAP/`; built by `SF40_PTCSubclassBranchSHAP/data_export.R`) |
...
```bash
cd ../SF40_PTCSubclassBranchSHAP && Rscript data_export.R
cd ../SF41_PTCSubclassPerformance && python3 figure_s_performance_by_subclass.py
```
...
- **`SF40_PTCSubclassBranchSHAP/`** — companion SF for the same paragraph;
