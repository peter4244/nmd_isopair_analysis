...
def render_length_kde(ax, tsv_name, n_label=None):
    ...
    ax.spines["bottom"].set_color(C_AXIS); ax.spines["bottom"].set_linewidth(0.8)
    for label, (xp, yp, line_c, n) in peaks.items():
        ax.text(xp, yp + y_max * 0.07,
                f"{label}\n(n={n:,})",
                ha="center", va="bottom",
                fontsize=BODY_FS - 2, color=line_c, fontweight="bold")
...
def render_kozak_violin(ax, tsv_name, pvalue_text):
    ...
    for s_ in ("bottom", "left"):
        ax.spines[s_].set_color(C_AXIS); ax.spines[s_].set_linewidth(0.8)
...
def render_position_hist(ax, tsv_name):
    ...
    counts, edges, _ = ax.hist(vals, bins=bins, color=C_HIST_FILL,
                                edgecolor=C_HIST_LINE, linewidth=0.6, alpha=0.85)
...
def build_figure():
    ...
    for ri in range(2):
        for ci in range(3):
            axes[ri, ci].text(-0.20, 1.04, letters[ri][ci],
                              transform=axes[ri, ci].transAxes,
                              fontsize=PANEL_LABEL_FS, fontweight="bold",
                              color=C_LBL, ha="left", va="bottom", clip_on=False)
