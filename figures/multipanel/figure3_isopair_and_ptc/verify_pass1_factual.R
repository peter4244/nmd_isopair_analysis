...
check("genes with >=1 NMD isoform", 5250, length(nmd_genes))
...
check("genes with >=2 non-NMD isoforms", 8220, length(genes_2plus_nonnmd))
genes_eligible <- intersect(nmd_genes, genes_2plus_nonnmd)
check("eligible genes (>=1 NMD AND >=2 non-NMD)", 2942, length(genes_eligible))
check("raw profiles_c2 pairs", 1548, nrow(profiles_c2))
check("raw profiles_c2 unique genes", 1548, length(unique(profiles_c2$gene_id)))
check("raw profiles_c4 pairs", 5001, nrow(profiles_c4))
...
check("pop_BC NMD pairs", 1548, nrow(pop_BC_c2))
check("pop_BC Control pairs", 1548, nrow(pop_BC_c4))
...
check("all-3-ENST NMD pairs (gene-matched)", 195, nrow(all3_c2))
check("all-3-ENST Control pairs (gene-matched)", 195, nrow(all3_c4))
...
check("all-3-ENST + coding-CDS, re-intersected NMD", 130, nrow(pop_DEF_c2))
check("all-3-ENST + coding-CDS, re-intersected Control", 130, nrow(pop_DEF_c4))
...
check("Panel D NMD n", 130, sum(panelD_tsv$comparison == "NMD"))
check("Panel D Control n", 130, sum(panelD_tsv$comparison == "Control"))
...
check("Panel D NMD median distance", -58.5, med_nmd)
check("Panel D Control median distance", -151, med_ctrl)
...
check("Panel D NMD PTC+ count (distance > 50)", 48, n_ptc_nmd)
check("Panel D Control PTC+ count (distance > 50)", 2, n_ptc_ctrl)

# Headline percentages
ptc_rate_nmd <- round(100 * n_ptc_nmd / nrow(pop_DEF_c2), 1)
ptc_rate_ctrl <- round(100 * n_ptc_ctrl / nrow(pop_DEF_c4), 1)
check("NMD PTC rate % (48/130)", 36.9, ptc_rate_nmd)
check("Control PTC rate % (2/130)", 1.5, ptc_rate_ctrl)

# Fold enrichment (integer)
fold <- round(n_ptc_nmd / n_ptc_ctrl)  # exact counts (equal denominators)
check("Fold enrichment (~24x)", 24, fold)
...
check("Panel D NMD clipped count (axis [-1000,1500])", 2, n_clip_nmd)
...
check("Panel E n_ptc_attr (sum n_ptc_events)", 48, n_ptc_attr)
...
check("Panel E n_ctrl_events sum in TSV (9 PTC-matching event types)",
      209, ctrl_total_tsv)
check("Panel F total attributed pairs (sum n)", 48, sum(panelF_tsv$n))

# Top event (SE)
se_row <- panelE_tsv[event_type == "SE"]
check("Panel E SE n_ptc_events", 23, se_row$n_ptc_events)
check("Panel E SE pct_of_ptc", 47.9, se_row$pct_of_ptc, tol = 0.1)
check("Panel E SE n_ctrl_events", 42, se_row$n_ctrl_events)
check("Panel E SE pct_ctrl", 14.4, se_row$pct_ctrl, tol = 0.1)
...
check("Panel E SE fisher_p ~ 6.88e-07", 6.88e-07, se_p, tol = 1e-08)

# A5SS row
a5_row <- panelE_tsv[event_type == "A5SS"]
check("Panel E A5SS n_ptc_events", 6, a5_row$n_ptc_events)
check("Panel E A5SS pct_of_ptc", 12.5, a5_row$pct_of_ptc, tol = 0.1)
check("Panel E A5SS n_ctrl_events", 12, a5_row$n_ctrl_events)
check("Panel E A5SS pct_ctrl", 4.1, a5_row$pct_ctrl, tol = 0.1)
check("Panel E A5SS direction", "Enriched in PTC-causing", a5_row$direction)
check("Panel E A5SS fisher_p ~ 2.83e-02", 2.83e-02, a5_row$fisher_p, tol = 1e-3)

# Alt_TES row (depleted)
at_row <- panelE_tsv[event_type == "Alt_TES"]
check("Panel E Alt_TES n_ptc_events", 2, at_row$n_ptc_events)
check("Panel E Alt_TES pct_of_ptc", 4.2, at_row$pct_of_ptc, tol = 0.1)
check("Panel E Alt_TES n_ctrl_events", 76, at_row$n_ctrl_events)
check("Panel E Alt_TES pct_ctrl", 26.0, at_row$pct_ctrl, tol = 0.1)
check("Panel E Alt_TES direction", "Depleted in PTC-causing", at_row$direction)
check("Panel E Alt_TES fisher_p ~ 3.24e-04", 3.24e-04, at_row$fisher_p, tol = 1e-5)

# Panel F mechanism totals (3 mechanisms only)
cat("  Panel F mechanism totals:\n")
mech_totals <- panelF_tsv[, .(n = sum(n)), by = mechanism]
print(mech_totals)
check("Panel F Frameshift total", 25, sum(panelF_tsv$n[panelF_tsv$mechanism == "Frameshift"]))
check("Panel F In-frame stop total", 18, sum(panelF_tsv$n[panelF_tsv$mechanism == "In-frame stop"]))
check("Panel F 3'UTR splice total", 5, sum(panelF_tsv$n[panelF_tsv$mechanism == "3'UTR splice"]))

# Mechanism percentages
fs_pct <- round(100 * 25 / 48, 1)
ifs_pct <- round(100 * 18 / 48, 1)
utr_pct <- round(100 * 5 / 48, 1)
check("Panel F Frameshift %", 52.1, fs_pct, tol = 0.1)
check("Panel F In-frame stop %", 37.5, ifs_pct, tol = 0.1)
check("Panel F 3'UTR splice %", 10.4, utr_pct, tol = 0.1)

# Top event count in Panel F
se_in_F <- sum(panelF_tsv$n[panelF_tsv$event_type == "SE"])
check("Panel F top event SE total", 23, se_in_F)

# ============================================================================
# Section F — Panel B + C (unchanged scope at pop_BC = 1,548 each)
# ============================================================================
cat("\n[F] Panel B + C (pop_BC = 1,548 each)\n")
panelB_tsv <- fread("data/panelB_sequence_similarity.tsv")
check("Panel B NMD n", 1548, sum(panelB_tsv$comparison == "NMD"))
check("Panel B Control n", 1548, sum(panelB_tsv$comparison == "Control"))
