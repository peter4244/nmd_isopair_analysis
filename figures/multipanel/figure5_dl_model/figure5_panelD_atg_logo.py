"""
Figure 5 Panel D — SHAP nucleotide logo across ±20 nt around the start codon.

Mirrors old Fig 6E ("SHAP values at start codon"): two stacked logos —
NMD on top, Control on bottom — with per-position A/C/G/T letters scaled
by |SHAP|. The model has learned the Kozak context: enrichment of A/G at
−3 and G at +4 visible in NMD.

Data:
  ./data/panelD_atg_profile.tsv (relative_position, channel, nmd_abs_shap,
  ctrl_abs_shap; channels = A,C,G,T,frame_*,junction,rolling_gc — we filter
  to A/C/G/T). Source: shap_profile_atg_joint_atg500_stop500.tsv (4ct).
...
def load_matrix(value_col):
    df = pd.read_csv(HERE.parent / "data" / "panelD_atg_profile.tsv", sep="\t")
...
def main():
    ...
    out_dir = HERE.parent
    fig.savefig(out_dir / "figure5_panelD_atg_logo.pdf", facecolor="white")
    fig.savefig(out_dir / "figure5_panelD_atg_logo.png", dpi=300, facecolor="white")
    print("Saved: figure5_panelD_atg_logo.pdf and .png")
