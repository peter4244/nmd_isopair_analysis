...
>
> The **AUG branch** and **stop branch** each process a 9 × 500 sequence-and-feature window through a shared-weight 1D convolutional stack: three convolutional blocks with kernel size 7 `[verify]`, stride 1, and 32 / 64 / 128 `[verify]` output channels (each block followed by batch normalization and a ReLU activation, with max-pooling of kernel size 4 / 4 / 2 `[verify]` between blocks), followed by adaptive average pooling to a fixed-length vector and a single dense projection to 28 dimensions `[verify]` per branch. The two branches do not share weights with each other but DO share weights across the five ORF slots within each branch.
>
> The **structural branch** is a per-ORF linear projection of the five hand-engineered structural features (`frac_start`, `frac_stop`, `is_ref_cds`, `is_sqanti_cds`, `n_downstream_ejc`) to an 8-dimensional `[verify]` embedding via a single fully-connected layer (no activation; bias enabled).
>
> The three branch outputs (28 + 28 + 8 = 64 dimensions `[verify]`) are concatenated per ORF. The five per-ORF embeddings are aggregated by a learned attention layer: a single dense projection from 64 → 1 produces per-ORF attention logits that are softmax-normalized across the five ORFs; the per-ORF embeddings are then weighted by the attention probabilities and summed to yield a single 64-dim transcript embedding. The transcript embedding feeds a two-layer MLP head (64 → 32 → 1 `[verify]`) with a ReLU non-linearity between the two dense layers; the final scalar is the NMD logit.

> **`[verify]` markers:** I drafted the per-branch CNN architecture from my best reading of the design intent. The actual layer counts, kernel sizes, and channel counts are in `NMD_orf_model_v5_4ct/model.py`, which I could not read this session (cluster down). Before pasting into the Google Doc, replace each `[verify]` placeholder with the actual value from `model.py`. If `model.py` deviates from this skeleton in a more fundamental way (e.g., different number of conv blocks, residual connections), rewrite the paragraph accordingly.
