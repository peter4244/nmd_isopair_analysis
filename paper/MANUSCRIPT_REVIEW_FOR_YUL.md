...
| **4.8** | **SF23 vs the text — would value your read on this one.** §3 ¶2 states "Spearman ≥ 0.96" for non-NMD genes, but the SF23 panels print AT2 0.97 · **LAE 0.95** · FB 0.97 · MV 0.97, so the LAE panel sits just under the stated floor. Recomputing on the current data gives AT2 0.9671 · **LAE 0.9643** · FB 0.9705 · MV 0.9661 — i.e. the text's ≥0.96 holds, and LAE rounds to 0.96 rather than 0.95. That pattern would fit SF23 having been rendered before a data update, but you'd know better than we would whether that's what happened. **Could you check which run SF23 reflects?** If it predates an update, re-rendering would bring the figure in line; if 0.95 is current, then the text's floor is the thing to adjust. Flagging it because a reviewer comparing the sentence to the figure it cites would notice. |
...
7. **§4.6** — is there a supplemental results document holding the cell-type-specific pathway
   findings?
8. **§4.8** — which run does SF23 reflect? Its LAE panel prints 0.95 against the text's
   "≥ 0.96" (we recompute 0.9643) — is that a stale render, or is 0.95 current and the text
   the thing to adjust?
