...
| **Table 3** | Top-5 **gene-level** significant pathways per CT (NES, p.adj, size) | **`code/upstream/Gene-Level_DGE_Summary_mashR.Rmd`** (msigdbr **26.1.0**, seed **42**) — NOT Pete's `gsea_mashr_2026.3.10.R`, whose output does not match. | ⚠️ **Yul re-ran with the seed and updated Table 3 in the manuscript** — re-verify against the current Doc. Local reproduction needs msigdbr 26.1.0 (have 25.1.1). |
| **Table 4** | Top-5 **isoform-level** significant pathways per CT | **`code/upstream/die_mashr_enrichment_part2_2026.3.10.Rmd`** (msigdbr 26.1.0, seed 42). **Gap CLOSED 2026-07-21 (Yul).** | Not yet re-derived; needs msigdbr 26.1.0. |
...
| 1.10 | ❓ **OPEN QUESTION FOR YUL — criterion unidentified (2026-07-21).** Replicated `ct_de.Rmd` end-to-end and **every checkable step matches**: filter → **105,938 exactly** (= claim 1.9); donor column = `id` (confirmed correct — `get_donor()` at line 97 prefers `id`); design, one-vs-rest contrasts, and the mashr recipe (seed 42, `n_random=20000`, `estimate_null_correlation_simple`, `cov_pca` + `cov_canonical`) all as written. **But no criterion in the file reproduces the counts,** and the two *directional* variants bracket the claim:
...
| 2.24 | ⚠️ **LIKELY VERSION ARTIFACT — re-check pending (2026-07-21).** AT2 had 12 CT-specific pathways in my run, none TRANSLATION/XENOBIOTIC. But my run used **msigdbr 25.1.1** while the tables used **26.1.0** (per Yul), so gene-set membership differs. **Not evidence of an error.** Re-run `Gene-Level_DGE_Summary_mashR.Rmd` under msigdbr 26.1.0 + seed 42 to settle it. *(Checked against `tmp/gsea_mashr_gene_2026.3.10_run2026-05-18.tsv`, which is NOT the run behind Tables 3–4 (see 2.19). Treat as a robustness signal across runs, not a definitive verification.)* AT2-specific: translation-regulatory programs, xenobiotic metabolism | Same. | Verifiable locally. |
| 2.25 | ⚠️ **LIKELY VERSION ARTIFACT — re-check pending (2026-07-21).** `GOBP_REGULATION_OF_RNA_SPLICING` was MV-only (padj 0.0055) in my msigdbr 25.1.1 run; the tables used 26.1.0. Same re-check as 2.24. *(Checked against `tmp/gsea_mashr_gene_2026.3.10_run2026-05-18.tsv`, which is NOT the run behind Tables 3–4 (see 2.19). Treat as a robustness signal across runs, not a definitive verification.)* AT2 + MV: regulation of RNA splicing | Same. | Verifiable locally. |
