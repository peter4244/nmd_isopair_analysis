...
| 1.13 | ⚠️ **FILTER NARROWED 2026-07-24 (`verify_section1_claim113.R`) — it is PER-CELL-TYPE.** Of nine candidates, per-CT `filterByExpr` is the only one reproducing the claim's ordering (LAE lowest 25.6, MV highest 45.0 vs claimed 27.8 / 51.7); every global filter gets the ordering wrong. Values remain 2–7 points low, so the exact definition is still needed — see **F-14**. ✅ **An apparent contradiction here was withdrawn 2026-07-24 (Pete):** cell-type-restricted expression is defined over a **larger universe than the 162,800 DIE isoforms** — a restricted isoform may by definition fail the expression filter — so the "% passed" and "% significant" denominators are different populations. Confirmed: defining restricted isoforms on the **unfiltered** DGEList and intersecting with the DIE universe reproduces 1.12's counts exactly (LAE 72,508→8,087; AT2 30,240→2,267; MV 39,456→2,015; FB 25,361→1,131). *(Superseded diagnosis 2026-07-21:* — taking "the additional expression filter" as the DMSO `filterByExpr` set (105,938) gives **LAE 19.5% → MV 31.4%** vs claimed **27.8% → 51.7%**. The *structure* reproduces (LAE lowest, MV highest, same ordering), so the restricted sets are right (1.12 counts are exact) but the second filter is something else. Not a discrepancy — the filter simply is not specified in the Methods. "27.8% (LAE) to 51.7% (MV) of CT-restricted isoforms passed the additional expression filter" | Same. | Cite-only. |
