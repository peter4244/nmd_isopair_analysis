...
| # | Claim | Source code | Notes / status |
|---|---|---|---|
| 2.19 | ⛔ **BLOCKED — GSEA PROVENANCE UNRESOLVED (2026-07-21).** The mapped output `nmd:tmp/gsea_mashr_gene_2026.3.10_run2026-05-18.tsv` **does not reproduce the manuscript pathway tables**: for AT2 "RNA splicing" it gives NES 1.47 / padj 0.016 / **size 449**, while Table 3 reports NES 1.49 / padj 0.018 / **size 462**. A differing gene-set size implies a different MSigDB version or gene universe, i.e. a different run. Claims 2.19–2.26 and Tables 3–4 therefore **cannot be verified against this file** and no discrepancy should be inferred from it. *(For the record, that file yields 2 pathways significant in 3 CTs and 3 in 2 CTs, padj 0.0007–0.0411 — reported only as evidence of mismatch, NOT as a correction to the claim.)* **→ Identify the GSEA run that produced Tables 3–4 before verifying §2 pathway claims.** "5 pathways significant in 3 of 4 CTs (LAE, FB, MV; FDR 0.002–0.048); AT2 trending (NES 1.65–1.83 below FDR)" — Table X — Pathways Significant | `nmd:code/gsea_mashr_2026.3.10.R` → output TSV `nmd:tmp/gsea_mashr_gene_2026.3.10_run2026-05-18.tsv` (per ONBOARDING §9). Input: signed mashr posterior-mean logFC ranking per CT from `nmd:shortread_dge/mashr/nmd_mashr_dge_*_2026.3.10.csv`. | **Verifiable locally** — both code and output are in the repo. |
...
| 2.24 | ⚠️ **DID NOT REPRODUCE in the available run (2026-07-21)** — AT2 has 12 CT-specific pathways there, but none match TRANSLATION or XENOBIOTIC. **Cannot distinguish a run difference from an error** until the correct GSEA run is identified (2.19). Do not treat as a discrepancy yet. *(Checked against `tmp/gsea_mashr_gene_2026.3.10_run2026-05-18.tsv`, which is NOT the run behind Tables 3–4 (see 2.19). Treat as a robustness signal across runs, not a definitive verification.)* AT2-specific: translation-regulatory programs, xenobiotic metabolism | Same. | Verifiable locally. |
| 2.25 | ⚠️ **PARTIAL in the available run (2026-07-21)** — `GOBP_REGULATION_OF_RNA_SPLICING` is significant in **MV only** (padj 0.0055), not AT2. Same caveat as 2.24: gated on identifying the correct run. *(Checked against `tmp/gsea_mashr_gene_2026.3.10_run2026-05-18.tsv`, which is NOT the run behind Tables 3–4 (see 2.19). Treat as a robustness signal across runs, not a definitive verification.)* AT2 + MV: regulation of RNA splicing | Same. | Verifiable locally. |
